...
/**
 * Modified By: Ahmad Milzam (ahmadmilzam.com)
 */
...
class MY_Model extends CI_Model
{
     ...
     * @param  [type] $string_fields [description]
     * created by: Ahmad Milzam (ahmadmilzam.com)
     */
    public function select($string_fields)
    {
        $this->_database->select($string_fields);
        return $this;
    }
    ...
    /**
    * Fetch all categories from database and
    * reformat to work with tree builder custom html simple_tree_builder helper easily
    * !! this is only works in adjency type list !!
    * columns need : id_category, id_parent, name
    */
    public function get_parent_child_tree()
    {
        $results = $this->as_array()->select($this->primary_key.', id_parent, name')->get_all();

        // First, re-index the array.
        foreach ($results as $row)
        {
          $array[$row[$this->primary_key]] = $row;
        }
        ...
        {
          if (array_key_exists($row['id_parent'], $array))
          {
            // Add this page to the children array of the parent page.
            $array[$row['id_parent']]['children'][] =& $array[$row[$this->primary_key]];
          }

          // This is a root page.
          if ($row['id_parent'] == 0)
          {
            $parent_child_array[] =& $array[$row[$this->primary_key]];
          }
        }

        return $parent_child_array;
    }
